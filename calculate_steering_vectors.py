...
def train_classifier(X, y, X_test, y_test, config_key, c_val, seed):
    ...
    if model_type == 'linearsvc':
        ...
        clf = SVC(kernel='rbf', C=c_val, gamma=cfg['gamma'], random_state=seed)
    elif model_type == 'rf':
    ...
    # 2. Fit model (using Calibration for Linear models if desired)
    if model_type in ['linearsvc', 'logistic']:
        # For linear models, we use calibration to get smoother scores
        clf = CalibratedClassifierCV(clf, cv=3).fit(X, y)
        # Extract mean coef_ across folds
    ...
    elif model_type in ['linear_no_penalty']:
        clf.fit(X, y)
        score = clf.score(X_test, y_test)
        coef = torch.from_numpy(clf.coef_[0]).float()
    else:
        # For non-linear models (RBF, RF), fit directly
        clf.fit(X, y)
        score = clf.score(X_test, y_test)
        # Non-linear models do not have a .coef_ attribute in input space
        coef = None 
...
def train_ensemble_svms_best_tokens(data_pos, data_neg, best_tokens, args):
    ...
    #print(f"\n--- Starting Ensemble Training (Penalty: {args.penalty}) ---")
...
def main():
    ...
    prefix = f"{args.save_dir}/{name}_{args.threshold}_{args.n_samples}"
    ...
    # if args.best_tokens:
    #     _, normals_pt, scores_pt, best_tokens = train_svms_per_token(
    #         data_pos, data_neg, args.timesteps, args.blocks, args.n_samples, args.threshold)
    #     if args.separate_normals:
    #         torch.save(normals_pt, f'{prefix}_normals_separate.pt')
    #         np.save(f'{prefix}_scores_separate.npy', scores_pt)
    #         torch.save(best_tokens, f'{prefix}_best_tokens.pt')
