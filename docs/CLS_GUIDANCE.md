...
Для следующего эксперимента после `figures/cls_guidance/` подготовлены два
`*_all_steps.json`. Они используют тот же mean diff и те же prompt/seed:
...
DINO получает непрерывный выход VAE до clipping/округления в PNG: снижение loss
может отражать особенности этого представления, а не видимое удаление очков.
