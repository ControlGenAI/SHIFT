...
from diffusers.pipelines.stable_diffusion_3.pipeline_stable_diffusion_3_img2img import\
    retrieve_latents, retrieve_timesteps
import numpy as np
        ...
        **args,
        ...
        if photo is not None:
            ...
            photo =  self.image_processor.preprocess(  # Center crop + resize
                photo, height=1024, width=1024, resize_mode="crop"
            )
            photo = photo.to(latents.dtype).to(latents.device)
            #photo = photo.mean(1, keepdim=True)
            #photo = photo.repeat(1,3,1,1)
            with torch.no_grad():
                init_latents = retrieve_latents(self.vae.encode(photo), generator=generator)
            ...
            for i, t in enumerate(timesteps):
                ...
                if photo is None:
                    ...
                    noise_pred = self.transformer(
                        hidden_states=latent_model_input,
                        timestep=timestep,
                        encoder_hidden_states=torch.cat([prompt_embeds, prompt_embeds, prompt_embeds]),
                        pooled_projections=torch.cat([pooled_prompt_embeds, pooled_prompt_embeds, pooled_prompt_embeds]),
                        joint_attention_kwargs=self.joint_attention_kwargs,
                        return_dict=False,
                    )[0]
        ...
        if output_type == "latent":
            ...
            if photo is not None:
                latents = torch.cat([latents, init_latents])
